...
public class RecipientEditTextView extends MultiAutoCompleteTextView implements
    ...
    private float mChipFontSizeSub;
        ...
        baseAdapter.registerUpdateObserver(new BaseRecipientAdapter.EntriesUpdatedObserver() {
            ...
            public void onChanged(List<RecipientEntry> entries) {
                ...
                if ((entries != null)
                    ...
                    setDropDownHeight(Math.max(0, mRect.bottom - mCoords[1] - mDropdownAnchor.getHeight() -
    ...
    private ChipBitmapContainer createChipBitmap(RecipientEntry contact, TextPaint paint,
        ...
        CharSequence ellipsizedTextTitle = ellipsizeText(createChipDisplayTextTitle(contact), paint,
                calculateAvailableWidth()
                    ...
                    - indicatorPadding);
        int textWidth = (int) paint.measureText(ellipsizedTextTitle, 0, ellipsizedTextTitle.length());

        CharSequence ellipsizedTextSub = null;
        String subTitle = createChipDisplayTextSub(contact);
        TextPaint paintSub = new TextPaint(paint);
        paintSub.setTextSize(mChipFontSizeSub);

        if (!TextUtils.isEmpty(subTitle)) {
            ellipsizedTextSub = ellipsizeText(subTitle, paintSub,
                    calculateAvailableWidth()
                    - iconWidth
                    - warningIconWidth
                    - warningIconEndMargin
                    ...
                    - backgroundPadding.right
                    - indicatorPadding);
            int textWidthSub = (int) paintSub.measureText(ellipsizedTextSub, 0, ellipsizedTextSub.length());
            textWidth = Math.max(textWidth, textWidthSub);
        }
        ...
        if (ellipsizedTextSub != null) {
            int textHeightTile = calculateTextHeight();
            int textHeightSub = calculateTextHeight(paintSub);
            int centerY = height / 2;
            int titleYOffsetTitle = centerY - (centerY - textHeightTile) / 2;
            int titleYOffsetSub = centerY + (centerY - textHeightSub) / 2;

            canvas.drawText(ellipsizedTextTitle, 0, ellipsizedTextTitle.length(),
                    textX, titleYOffsetTitle, paint);

            canvas.drawText(ellipsizedTextSub, 0, ellipsizedTextSub.length(),
                    textX, titleYOffsetSub + textHeightSub, paintSub);
        } else {
    ...
    private void loadAvatarIcon(final RecipientEntry contact,
        ...
        boolean drawPhotos = true; /*isPhoneQuery() ?
    ...
    private void setChipDimensions(Context context, AttributeSet attrs) {
        ...
        if (mChipTextStartPadding == -1) {
            mChipTextStartPadding = mChipTextEndPadding =
                    (int) r.getDimensionPixelSize(R.dimen.chip_padding);
        }
        // xml-overrides for each individual padding
        // TODO: add these to attr?
        int overridePadding = (int) r.getDimensionPixelSize(R.dimen.chip_padding_start);
        if (overridePadding >= 0) {
            mChipTextStartPadding = overridePadding;
        }
        overridePadding = (int) r.getDimensionPixelSize(R.dimen.chip_padding_end);
        if (overridePadding >= 0) {
        ...
        if (mChipHeight == -1) {
            mChipHeight = r.getDimensionPixelSize(R.dimen.chip_height);
        }
        mChipFontSize = a.getDimensionPixelSize(R.styleable.RecipientEditTextView_chipFontSize, -1);
        if (mChipFontSize == -1) {
            mChipFontSize = r.getDimensionPixelSize(R.dimen.chip_text_size);
        }
        mChipFontSizeSub = r.getDimensionPixelSize(R.dimen.chip_text_size_sub);
        mAvatarPosition =
    ...
    String createChipDisplayTextTitle(RecipientEntry entry) {
        String display = entry.getDisplayName();
        String address = entry.getDestination();
        if (TextUtils.isEmpty(display) || TextUtils.equals(display, address)) {
            display = null;
    ...
    String createChipDisplayTextSub(RecipientEntry entry) {
        String display = entry.getDisplayName();
        String address = entry.getDestination();
        if (TextUtils.isEmpty(display) || TextUtils.equals(display, address)) {
            display = null;
